...
function closeOpenSwipe() {
  ...
  // 他の場所タップで閉じる（開いているときだけ）
  // ※ mainにだけ付けると外側タップが拾えないので、documentで閉じる
  // ただし複数回登録しないように、最初の1回だけ登録する
  if (!window.__wishSwipeDocBound) {
    window.__wishSwipeDocBound = true;
  ...
  // まず「縦スクロールか横スワイプか」を厳しめに判定する
  // 縦が少しでも優勢なら、何もしない（＝ブラウザの縦スクロールに任せる）
  if (!swiping) {
    // ある程度動くまで判定しない（小さいブレで殺さない）
    const ax = Math.abs(dx);
    const ay = Math.abs(dy);
    if (ax < 24 && ay < 24) return;

    // 縦が優勢 or ほぼ同じ → 縦スクロール扱い
    // (横が縦の1.35倍以上じゃない限り横と認めない)
    if (!(ax > ay * 1.35)) {
      tracking = false; // 以降、この指の動きはブラウザに任せる
      return;
    }
  ...
  if (row.classList.contains("open")) {
    // open中：右へ戻すを許可
    shift = Math.max(-ACTION_W, Math.min(0, -ACTION_W + dx));
  } else {
    // closed中：左へ開く（dx<0）だけ
    shift = Math.max(-ACTION_W, Math.min(0, dx));
  }
  ...
  main.addEventListener("touchend", () => {
    ...
    // 判定：一定以上左へ動いたらopen
    const shouldOpen = (!row.classList.contains("open") && dx < -40)
                    || (row.classList.contains("open") && dx < -10);

    // open中に右へ戻したらclose
    const shouldClose = row.classList.contains("open") && dx > 35;
